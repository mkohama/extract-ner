...
- 確定 … **実辞書（名簿）一致** または **連絡先の正規表現（決定的検出）**。昇格（session）は
  確定にしない＝確定の出所は実辞書か正規表現（どちらも確実）。
...
def _contact_candidates(text: str) -> list[Candidate]:
    """連絡先（メール等）を正規表現で検出し、**1 件まるごと**の確定候補にする。

    finditer で全出現を個別に拾う（＝展開不要で全部マスクされる）。確信度は **確定**：
    正規表現は「形」での決定的検出で、名簿一致と同等に確実なため（確定＝実辞書 ∪ 正規表現）。
    """
    out: list[Candidate] = []
